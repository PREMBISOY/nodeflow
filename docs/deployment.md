...
Create one Supabase PostgreSQL project for the whole team, set its pooled PostgreSQL URL as `DATABASE_URL` in Railway, then run migrations `001_project_knowledge.sql`, `002_platform_identity_tenants.sql`, and `003_database_auth.sql` in order. Do not commit credentials.
