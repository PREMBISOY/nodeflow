...
Create one Supabase PostgreSQL project for the whole team, set its pooled PostgreSQL URL as `DATABASE_URL` in Railway, then run migrations `001_project_knowledge.sql` through `004_team_github_repositories.sql` in order. Do not commit credentials.
...
Set `GITHUB_OAUTH_CLIENT_ID` and `GITHUB_OAUTH_CLIENT_SECRET` in Railway. The GitHub OAuth App callback URL must be `https://nodeflow.up.railway.app/api/v1/auth/github/callback` (or the matching value of `GITHUB_OAUTH_REDIRECT_URI`). GitHub sign-in uses only `read:user` and `user:email`; its access token is used only during sign-in and is never stored. A team member can connect a public `owner/repository` to a project with `POST /api/v1/teams/{team_id}/github/repositories`. The route verifies that the repository is public and requires the active, authorized team.
