...
export default function App() { const [me, setMe] = useState(); const [teams, setTeams] = useState([]); const [team, setTeam] = useState(); const [pendingTeam, setPendingTeam] = useState(); const [projects, setProjects] = useState([]); const [project, setProject] = useState(); const [context, setContext] = useState(); const [page, setPage] = useState('Overview'); const [error, setError] = useState(); const [showMembers, setShowMembers] = useState(false); const [teamCode, setTeamCode] = useState(''); const [showMembers, setShowMembers] = useState(false); const [teamCode, setTeamCode] = useState(''); const logout = () => { localStorage.removeItem('nodeflow.session'); setMe(); setTeams([]); setTeam(); setProjects([]); setProject(); setContext() }; const reloadContext = async () => { if (!project) return; try { setError(); setContext(await api.context(project.id)) } catch (err) { setError(err) } }; const start = async () => { try { setError(); const current = await api.me(); const visibleTeams = dedupeTeams(current.teams, current.active_team_id); setMe(current.user); setTeams(visibleTeams); setTeam(visibleTeams.find(t => t.id === current.active_team_id) || null) } catch (err) { if (err instanceof ApiError && err.status === 401) logout(); else setError(err) } }; const selectTeam = async chosen => { if (!chosen) return; try { setError(); const switched = await api.switchTeam(chosen.id); localStorage.setItem('nodeflow.session', switched.access_token); setTeam(chosen); setProjects([]); setProject(); setContext() } catch (err) { setError(err) } }; const completeTeam = async created => { try { const all = await api.teams(); const updated = dedupeTeams(all.some(t => t.id === created.id) ? all : [...all, created], created.id); setTeams(updated); await selectTeam(updated.find(t => t.id === created.id) || created) } catch (err) { setError(err) } }; useEffect(() => { const token = new URLSearchParams(window.location.hash.slice(1)).get('access_token'); if (token) { localStorage.setItem('nodeflow.session', token); window.history.replaceState({}, '', window.location.pathname) } if (localStorage.getItem('nodeflow.session')) start() }, []); useEffect(() => { if (team) { api.projects(team.id).then(items => { setError(); setProjects(items); setProject(items[0] || null) }).catch(setError); api.teamJoinCode(team.id).then(res => setTeamCode(res.join_code || res.team_code)).catch(() => setTeamCode('')); } }, [team?.id]); useEffect(() => { if (project) api.context(project.id).then(value => { setError(); setContext(value) }).catch(setError) }, [project?.id]); if (!me) return <Auth done={start} />; if (pendingTeam) return <TeamCode team={pendingTeam} continueToProject={async () => { const created = pendingTeam; setPendingTeam(); await completeTeam(created) }} />; if (!teams.length || !team) return <TeamForm finish={completeTeam} created={setPendingTeam} />; if (!project && projects.length === 0) return <ProjectForm team={team} done={created => { setProjects([created]); setProject(created) }} />; return <div className="app"><aside><div className="brand"><b>n</b> nodeflow</div><label className="team-label">ACTIVE TEAM<select value={team.id} onChange={e => selectTeam(teams.find(t => t.id === e.target.value))}>{teams.map(t => <option key={t.id} value={t.id}>{t.name}</option>)}</select></label><label className="team-label">TEAM CODE<div className="team-code-display">{teamCode || 'Hidden'}</div></label><label className="team-label">TEAM CODE<div className="team-code-display">{teamCode || 'Hidden'}</div></label><label className="team-label">PROJECT<select value={project?.id || ''} onChange={e => { setContext(); setProject(projects.find(p => p.id === e.target.value)) }}>{projects.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><GitHubRepository team={team} project={project} synced={reloadContext} /><div className="team-actions"><button onClick={() => setTeam(null)}>＋ Create / Join team</button></div><div style={{padding:'0 9px 10px'}}><button className="link" style={{width:'100%', textAlign:'left', padding:'10px', background:'#292a2f', borderRadius:'8px', color:'#fff'}} onClick={() => setShowMembers(true)}> Team Members</button></div><div style={{padding:'0 9px 10px'}}><button className="link" style={{width:'100%', textAlign:'left', padding:'10px', background:'#292a2f', borderRadius:'8px', color:'#fff'}} onClick={() => setShowMembers(true)}>+ Team Members</button></div><nav>{pages.map(item => <button key={item} className={item === page ? 'active' : ''} onClick={() => setPage(item)}>{item}</button>)}</nav><footer><button className="link" onClick={logout}>Log out</button><small>{me.email}</small></footer></aside><main><header><span>{team.name} / <b>{project?.name}</b></span></header><article>{error ? <ErrorBox error={error} retry={start} /> : context ? <Brain context={context} page={page} /> : <div className="loading">Loading authorized project context…</div>}</article></main>{showMembers && <TeamMembersModal team={team} onClose={() => setShowMembers(false)} />}</div> }
